...
/// @dev A contract for deploying and configuring a minimal Router contract.
contract InitialMigration {
    ...
    /// @dev The allowed caller of `initializeRouter()`. In production, this would be
    ///      the governor.
    ...
    /// @dev Initialize the `Router` contract with the minimum feature set,
        ...
        returns (Router _router)
    {
        ...
        IBootstrapFeature(address(router)).bootstrap(
            address(this),
    ...
    /// @dev Sets up the initial state of the `Router` contract.
    ///      The `Router` contract will delegatecall into this function.
    /// @param owner The new owner of the Router contract.
    /// @param features Features to bootstrap into the proxy.
